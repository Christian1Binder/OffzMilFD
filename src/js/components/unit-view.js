...
export function renderUnit(container, moduleId, unitId) {
    ...
    let html = `
        ...
        <div id="unit-controls" style="margin-top: 20px; display: flex; justify-content: space-between;">
            <button id="prev-btn" class="btn btn-secondary" style="width: 48%;">Previous</button>
            <button id="next-btn" class="btn" style="width: 48%;">Next</button>
        </div>
    `;
            ...
            case 'flashcard':
                cardHtml = `
                    <div class="card fade-in flashcard-container" style="perspective: 1000px; cursor: pointer;">
                        <div class="flashcard-inner" style="position: relative; width: 100%; height: 200px; text-align: center; transition: transform 0.6s; transform-style: preserve-3d;">
                            <div class="flashcard-front" style="position: absolute; width: 100%; height: 100%; backface-visibility: hidden; background-color: var(--color-panel); display: flex; align-items: center; justify-content: center; border: 1px solid var(--color-primary);">
                                <h3>${card.front}</h3>
                                <p class="text-dim">(Tap to Flip)</p>
                            </div>
                            <div class="flashcard-back" style="position: absolute; width: 100%; height: 100%; backface-visibility: hidden; background-color: var(--color-primary); color: var(--color-bg); transform: rotateY(180deg); display: flex; align-items: center; justify-content: center;">
                                <h3>${card.back}</h3>
                            </div>
                        </div>
                    </div>
        ...
        if (flashcard) {
            flashcard.addEventListener('click', () => {
                flashcard.querySelector('.flashcard-inner').style.transform =
                    flashcard.querySelector('.flashcard-inner').style.transform === 'rotateY(180deg)' ? 'rotateY(0deg)' : 'rotateY(180deg)';

                // XP Reward for interaction (Once per card per session)
